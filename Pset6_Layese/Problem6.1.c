...
int main(void){
 ...
 //Compute Triangle Wave (interleave channels)
  for(int t = 0; t < kNumFrames; t++){
    for(int i = 1; i <= numHarmonics; i+=2 ){ //Up to numHarmonics but skips every other harmonic
      double sample = amplitude/i * cos(2.0 * M_PI * ((frequency * i)/kSampleRate) * t); //replaced sin with cos
