...
#include <stdio.h>
#include <stdlib.h>
//Used null pointers because the variables haven't been assigned yet

//de allocation of memory from previous iteration
...
//Just like random walk in Tone Matrix
...
//typedef that holds the null terminated array 
NTArray* createArray() {
...
NTArray* buildKey(NTArray* words, int depth) {
	NTArray* key = createArray();
	CHECK(key != NULL, NULL);
	int len = ntLen(words->data) - 1; // Without the 1 it'd give you an extra word
